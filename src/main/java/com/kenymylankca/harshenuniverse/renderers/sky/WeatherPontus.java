package com.kenymylankca.harshenuniverse.renderers.sky;
...
{    
	...
	{
		if(world.getBiome(mc.getRenderViewEntity().getPosition()) instanceof BasePontusResourceBiome)
		{
			update += 1f;
	        ...
	        for (int k1 = k - t; k1 <= k + t; ++k1)
	        {
	            for (int l1 = i - t; l1 <= i + t; ++l1)
	            {
	                ...
	                BasePontusResourceBiome biome = (BasePontusResourceBiome) world.getBiome(mc.getRenderViewEntity().getPosition());
	                ...
	                if(!HarshenUtils.isLevelAcceptable(mc.world, mutableBlockPos, mc.player))
	                {
	                	if (j1 != 1)
	                    {
	                        ...
	                        mc.getTextureManager().bindTexture(new ResourceLocation(HarshenUniverse.MODID, "textures/biome-walls/" + biome.getBiomeName().toLowerCase() + ".png"));
	                        j1 = 1;
